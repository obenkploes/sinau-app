...
const Jurusan = () => {
    ...
    // handle notifikasi
    const handleNotifikasi = (show, title, message) => {
        setNotif({
            show: show,
            title: title,
            message: message
    ...
    const handleSubmit = async (e) => {
        ...
        await axios({
            ...
            .then(res => {
                let list = listJurusan
                if (jurusan.id>0) {
                    let index = list.findIndex(el=>el.id==jurusan.id)
                    list[index]=res.data.jurusan
                    setListJurusan(list)
                    handleNotifikasi(true,'Informasi','Data berhasil diperbaharui')
                }else{
                    list.push(res.data.jurusan)
                    setListJurusan(list)
                    handleNotifikasi(true,'Informasi','Data berhasil ditambahkan')
                }
                handleCancel()
            })
    ...
    return (
        <>
            ...
            <Notifikasi showToast={notif.show} close={()=>handleNotifikasi(false,'','')} title={notif.title} message={notif.message}/>
        </>
    )
}
